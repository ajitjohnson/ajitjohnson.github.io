...
Hey guys, welcome back to another tutorial on **data-analysis and programming with Python and R**. I hope you are already setup with *Jupyter Notebook* to run your first python program. If not, you can checkout my [previous article](/_posts/2020-07-14-setup-jupyter-notebook-python-R.markdown) on how to set up a *jupyter notebook* for Python.

> **Download the Jupyter Notebook for this [tutorial here](assets/data/2020-07-15-python-for-beginners-first-program.ipynb)**
...
Since python recognized that both the variables contains just text, it literally stitched them together as seen above.

**What happens if you add a integer and a text togeather?. Do you think python will throw an error? Run it in your jupyter notebook and find out.**
...
## A more *fun* function
Let's write a function to identify the number of days left for your birthday. Unlike the previous two functions, you will notice that there is an extra that says with `import datetime`. These are called packages or modules.

> A module is a piece of software that has a specific functionality and are imported using the `import` command.

There are 100,000+ modules available for python. These are written by python users such as you and me and have decided to make it available to the community. The cool thing about these packages is that you can simply import them and use them without having re-write the code.

For example, remember we wrote the `add` function to sum two numbers above? There is a package called `numpy` that has a function called `sum` that does the exact same thing, so in a way we could have simply imported that packaged and used the `sum` function without having to write our own function.  

Anyway let's get into our days to birthday calculator. First let's import the `datetime` package which contains a number of useful function to work with dates.

You can simply import it as `import datetime`, however you can see below that I performed the import by using `import datetime as dt`. The `as` command is optional and is just a matter of convenience. In python, anytime you want to use a function from a package, you will need to be explicit. For example, if you would like to use the `date` function from the `datetime` module, you code it as `datetime.date()`. However, if you are lazy like me and would like to keep it short, you can use the `as` command during import and then re-write the above command as `dt.date()`.

Okay let's first import the package as below.
...
We save today's date into a variable called `now`. You can extract individual components such as `date`, `month` or `year` from the `now` variable as shown below.
...
- Step 4: Set the date based on the input parameters.
- Step 5: Subtract the future date from today's date to calculate the number of days in between.
- Step 6: Return the result.

You would have noticed that we did not have to pass in any information about the number of days in any given month and so on. This is the power of using the `datetime` package. It already knows. If we had not used it, we would have to write all those computations ourself.
...
Finally, below is a challenge for you. **I cannot stress how important this is.** The only way to learn to code is to  practice continuously. Go ahead and [download]((assets/data/2020-07-15-python-for-beginners-first-program.ipynb)) this notebook, execute the previous commands and complete the challenge below. If you get stuck at any point, drop me a comment below or if you successfully completed the task, let me know that in the comments as well.
...
Hope you enjoyed the this article on the **Data analysis and Programming Series**. I will continue the series, and if you would like me to drop you an email on publication, leave your email below.
